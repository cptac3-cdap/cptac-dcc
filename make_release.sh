...
( cd dist; md5sum cptacdcc-$VER.*.{tgz,zip,exe} > cptacdcc-$VER.md5 ; touch cptacdcc-$VER.txt )
if [ "$1" ]; then 
  for comment in "$@"; do 
...
gh release create "$DCCVER" dist/cptacdcc-$VER.*.{tgz,zip,exe} dist/cptacdcc-$VER.md5 -F dist/cptacdcc-$VER.txt -t "$DCCVER"
for a in dist/cptacdcc-$VER.*.{tgz,zip,exe}; do
  a1=`basename $a`
